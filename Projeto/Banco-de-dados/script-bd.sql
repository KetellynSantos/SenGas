...
INSERT INTO sensor (titulo) VALUES
	...
	('Sensor tubulação'),
	('Sensor armazenagem');
SELECT nomeFantasia AS 'Nome Fantasia', concat(razaoSocial, ' ', cnpj) AS 'Razão social e CNPJ', contratoAtivo AS 'Contrato Ativo' FROM empresa;
SELECT titulo AS Setor FROM setor;
SELECT concat(logradouro, ', ', numero, ' - ', cidade, ' - ' , uf, ' - ', cep) AS 'Endereço completo', complemento AS Complemento FROM endereco;
SELECT concat(nome, ' ', sobrenome) AS 'Dados do funcionário (Nome completo)', concat(telefone, ' / ', email, ' / ', senha) AS 'Dados de acesso (telefne, e-mail e senha)', nivelAcesso AS 'Nivel de acesso' FROM usuario;
SELECT idSensor AS Sensor, dtRegistro AS 'Data e horario da detecção' FROM captura;